...
/*
    This text box connects to a store and sends an extraparam using the delayed task
    so that all you have to do to set this up is to place the id of the store
    in the spStoreObj field and the name of the extraParam that you would like to set
    in the spParamName field, the UX below will do the rest.
*/
    ...
    onRender: function(ct, position){
        Ext.ux.ClearableSearchText.superclass.onRender.call(this, ct, position);
        this.triggerEl.on('mouseover',function(t){this.showSpTip(this.triggerTip,this.triggerEl)},this);
        this.inputEl.on('mouseover',function(t){this.showSpTip(this.qtip,this.inputEl)},this);
    },
    onTriggerClick : function() {
      this.reset();
        if(this.connectToStore!=='none' && this.useExtraParam!=='none'){
            Ext.getCmp(this.connectToStore).store.setExtraParam(this.useExtraParam,'');
    ...
    delaytask:function(){
    //  alert(this.getValue());
        Ext.getCmp(this.connectToStore).store.setExtraParam(this.useExtraParam,this.getValue());
        Ext.getCmp(this.connectToStore).store.load();   
    },
    onKeyUp : function(){
        if(this.connectToStore!=='none' && this.useExtraParam!=='none'){
...
Ext.define('Ext.ux.styledMultiCombo', {
    extend: 'Ext.form.field.ComboBox',
    alias: 'widget.smcbo',
    spmaxGrow:'',
    spmaxHeight:'',
    multiSelect:true,
    // Convert the input box back to the div so you can add div elements 
    ...
    onRender: function(ct, position){
        Ext.ux.styledMultiCombo.superclass.onRender.call(this, ct, position);
        this.box = Ext.get(this.getInputId());
        this.el_width = this.getEl().getWidth() + 10;
        
            // Set the default value or the selected values
            if(this.getRawValue() == ''){
    ...
    listeners:{
        select:{fn:function(){  
                var sel_text = this.getRawValue().split(',');
                styledMultiCombo_item_tpl.overwrite( 'input-display','');
                Ext.Array.each(sel_text, function(name, index){
                    styledMultiCombo_item_tpl.insertFirst('input-display',{value:name,id:index});       
                }); 
                 // this.box.update(this.sel_text);
                var text_width = this.el.getTextWidth(this.getRawValue());
                  if (this.el_width < text_width){
                      this.setSize(text_width + 30);
                  } else {
                      this.setSize(this.el_width + 30);
             }    
        }
      }
    }
  ...
  // data.
    ...
    idProperty:'cpkid',
      fields: [
       {name:'cpkid',       type: 'int'},
       {name:'check_num' ,  type: 'int'},
       {name:'check_amount',type: 'float'},
       {name:'check_date',  type: 'string'},
       {name:'check_pay_to',type: 'string'},
       {name:'check_state', type: 'string'},
       {name:'check_zip',   type: 'string'}
       ]
     }); 
 // This is the data model for the check register application
  // data.
...
var state_data_store = Ext.create('Ext.data.Store', {
 ...
 var search_check_state = Ext.create('Ext.ux.styledMultiCombo', {
   ...
   valueField: 'check_state',
    ...
    check_register_grid.getStore().setExtraParam('check_state',this.getValue());
 ...
 var check_register_grid = Ext.create('Ext.grid.Panel', {
  ...
  title:'Check Register Data.',
        ...
        columns: [ 
     {text:'Number',  sortable:true,   width:75,  dataIndex: 'check_num'},
     {text:'Amount', sortable:true, align:'right',renderer: Ext.util.Format.usMoney,  width:75, dataIndex: 'check_amount',   summaryType: 'sum',summaryRenderer: Ext.util.Format.usMoney },
     {text:'Date',  sortable:true,  renderer: Ext.util.Format.dateRenderer('Y-m-d'),  dataIndex: 'check_date' },
     {text:'Pay To', sortable:true,    flex:1,      dataIndex: 'check_pay_to'  },
     {text:'State',  sortable:true,    dataIndex: 'check_state'   },
     {text:'Zip',sortable:true,  dataIndex:'check_zip'}
    ],
    tbar:['-',search_check_pay_to,'-',search_check_state,'-'],
    dockedItems: [{
        xtype: 'pagingtoolbar',
        store: check_data_store,   // same store GridPanel is using
        dock: 'bottom',
        displayInfo: true,
        listeners:{
